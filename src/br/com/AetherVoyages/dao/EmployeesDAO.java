...
public class EmployeesDAO {
     ...
     public void saveEmp(Employees emp){
         try {
             String sql = "INSERT INTO tab_employees(nome, sobrenome, rg, cpf, sexo, nascimento, cargo)"
                     + " values(?, ?, ?, ?, ?, ?, ?)";
             ...
             stmt.setString(6, emp.getBirthDay());
             stmt.setString(7, emp.getOffice());
             stmt.execute();
             stmt.close();
     ...
     public void updateEmp(Employees emp){
         try {
             String sql = "UPDATE tab_employees SET nome=?, sobrenome=?, rg=?, cpf=?, sexo=?, nascimento=?, cargo"
                        + " WHERE =  id = ?";
             PreparedStatement stmt = con.prepareStatement(sql);
             stmt.setString(1, emp.getName());
             stmt.setString(2, emp.getLastName());
     ...
     public List<Employees> listEmp(){
          try{
            ...
            while(result.next()){
                ...
                emp.setSex(result.getString("sexo"));
